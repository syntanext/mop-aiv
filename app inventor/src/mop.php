  ...
  function __construct()
  {
    ...
    private function csv()
    {
        ...
        if ($this->driver === 'PDO')
        {
            ...
            /*
            *   If there is a result from the query then
            *   making the first row of csv as the number of rows
            */
            if($count > 0)
            {
                for ($a=0; $a < $count; $a++)
            ...
            // If no result then only set csv as number of affected or selected row
            else
            {
            $csv .= "\"$num_of_rows\""."\n";
            }
            ...
            foreach ($this->pdo_query as $column => $value)
            {
                for ($b=0; $b < $count; $b++)
                {   
                    // If there is a null column then make a null
                    if ($value[$b] === NULL)
                    {
                        $csv .= 'NULL'.",";
                    }
                    // else if is empty then make a empty
                    elseif ($value[$b] === '')
                    {
                        $csv .= "\"\"".",";
                    }
            
                    // setting the column result
                    else
                    {
                        $col = $value[$b];
                        $col = str_replace("\"","\"\"",$col);
